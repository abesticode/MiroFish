"""Zep Graph paging reading tool.

Zep's node/edge list interface uses UUID cursor for paging.
This module encapsulates automatic page turning logic (including single-page retry) and returns the complete list transparently to the caller."""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request, exponential backoff and retry when failed. Only retry network/IO type transient errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Acquire graph nodes in pages, and return max_items at most (default 2000). Each page request comes with its own retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Get all the edges of the graph in pages and return the complete list. Each page request comes with its own retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
