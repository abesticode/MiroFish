...
INTERVIEW_PROMPT_PREFIX = "Combine your personality, all past memories and actions, and reply to me directly with text without using any tools:"
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        if status in prepared_statuses and config_generated:
            ...
            logger.info(f"Simulation {simulation_id} detection result: ready (status={status}, config_generated={config_generated})")
            return True, {
                "status": status,
            ...
            logger.warning(f"Simulation {simulation_id} detection result: Not ready to complete (status={status}, config_generated={config_generated})")
            return False, {
                "reason": f"The status is not in the prepared list or config_generated is false: status={status}, config_generated={config_generated}",
                "status": status,
                "config_generated": config_generated
            }
...
def prepare_simulation():
    ...
    try:
        ...
        if not force_regenerate:
            logger.debug(f"Check if simulation {simulation_id} is ready to complete...")
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            logger.debug(f"Check results: is_prepared={is_prepared}, prepare_info={prepare_info}")
            if is_prepared:
                logger.info(f"Simulation {simulation_id} is ready, skip repeated generation")
                return jsonify({
                    "success": True,
                    "data": {
                ...
                logger.info(f"The simulation {simulation_id} is not ready to be completed, and the preparation task will be started.")
        ...
        try:
            ...
            logger.info(f"Expected number of entities: {filtered_preview.filtered_count}, types: {filtered_preview.entity_types}")
        except Exception as e:
        ...
        # Update simulation state (contains prefetched number of entities)
...
def get_prepare_status():
    ...
    try:
        ...
        # If there is no task_id, an error is returned
        if not task_id:
            if simulation_id:
                # There is simulation_id but it is not ready to be completed
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
...
def get_simulation_profiles(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain Profile: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain Profile in real time: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain Config in real time: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def start_simulation():
    ...
    try:
        ...
        enable_graph_memory_update = data.get('enable_graph_memory_update', False)  # Optional: Whether to enable map memory update
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if force:
                    logger.info(f"Force mode: clean simulation log {simulation_id}")
                    cleanup_result = SimulationRunner.cleanup_simulation_logs(simulation_id)
                    if not cleanup_result.get("success"):
                        logger.warning(f"Warnings when cleaning logs: {cleanup_result.get('errors')}")
                    force_restarted = True

                # The process does not exist or has ended. The reset status is ready.
                logger.info(f"Simulation {simulation_id} preparation has been completed and the reset state is ready (original state: {state.status.value})")
...
def get_agent_stats(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain Agent statistics: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_interview_history():
    ...
    try:
        ...
        logger.error(f"Failed to obtain Interview history: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def close_simulation_env():
    ...
    Send a shutdown environment command to the simulation, causing it to gracefully exit wait-for-command mode.
