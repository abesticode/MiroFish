...
Manage parallel simulations of Twitter and Reddit dual platforms
...
class SimulationStatus(str, Enum):
    ...
    RUNNING = "running"
    PAUSED = "paused"
    STOPPED = "stopped" # The simulation is stopped manually
    COMPLETED = "completed" # Simulate natural completion
    FAILED = "failed"
...
class SimulationState:
    ...
    #Configure generation information
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Full status dictionary (internal use)"""
        return {
    ...
    def to_simple_dict(self) -> Dict[str, Any]:
        """Simplified status dictionary (used by API return)"""
        return {
            "simulation_id": self.simulation_id,
            "project_id": self.project_id,
            "graph_id": self.graph_id,
...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        Args:
            ...
            simulation_requirement: simulation requirement description (used for LLM generation configuration)
            document_text: original document content (used for LLM understanding background)
            ...
            parallel_profile_count: The number of profiles generated in parallel, default 3
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No entities matching the conditions were found, please check whether the graph is constructed correctly"
                self._save_simulation_state(state)
                return state
            ...
            # Pass in graph_id to enable Zep retrieval function to obtain richer context
            ...
            #Set the file path for real-time saving (preferably using Reddit JSON format)
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                realtime_output_path=realtime_output_path, # Real-time saving path
                output_platform=realtime_platform #Output format
            )
            
            state.profiles_count = len(profiles)
            
            # Save the Profile file (note: Twitter uses CSV format, Reddit uses JSON format)
            # Reddit has been saved in real time during the generation process. Save it again here to ensure completeness.
            if progress_callback:
                progress_callback(
            ...
            # Note: The running script remains in the backend/scripts/ directory and is no longer copied to the simulation directory.
            # When starting the simulation, simulation_runner will run the script from the scripts/ directory
