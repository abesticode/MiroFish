...
def _to_pascal_case(name: str) -> str:
    """Convert names in any format to PascalCase (such as 'works_for' -> 'WorksFor', 'person' -> 'Person')"""
    # Split by non-alphanumeric characters
    parts = re.split(r'[^a-zA-Z0-9]+', name)
    # Then split by camelCase boundary (such as 'camelCase' -> ['camel', 'Case'])
    words = []
    for part in parts:
...
```json
{
    ...
    "analysis_summary": "A brief analysis of the text content"
}
```
...
- Person: any natural person (used when it does not fall into the specific types above)
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Verify entity type
        # Record the mapping of the original name to PascalCase for subsequent correction of edge’s source_targets reference
        entity_name_map = {}
        for entity in result["entity_types"]:
            # Force the entity name to be converted to PascalCase (required by Zep API)
            if "name" in entity:
        ...
        for edge in result["edge_types"]:
            # Force edge name to be converted to SCREAMING_SNAKE_CASE (required by Zep API)
            if "name" in edge:
                original_name = edge["name"]
                edge["name"] = original_name.upper()
                if edge["name"] != original_name:
                    logger.warning(f"Edge type name '{original_name}' auto-converted to '{edge['name']}'")
            # Correct the entity name reference in source_targets to be consistent with the converted PascalCase
        ...
        # Deduplication: Deduplication by name, keeping the first occurrence
        seen_names = set()
        deduped = []
        for entity in result["entity_types"]:
