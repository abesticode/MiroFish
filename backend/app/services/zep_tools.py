...
class SearchResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [f"Search query: {self.query}", f"{self.total_count} related information found"]
    ...
    def to_text(self) -> str:
        ...
        if self.entity_insights:
            ...
            for entity in self.entity_insights:
                text_parts.append(f"- **{entity.get('name', 'unknown')}** ({entity.get('type', 'actual')})")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        #Historical/expired facts (complete output, not truncated)
...
class AgentInterview:
    ...
    agent_name: str
    agent_role: str # Role type (such as: student, teacher, media, etc.)
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                #Truncate overly long content (truncate by periods instead of hard truncation)
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        """Convert to detailed text format for LLM to understand and report for reference"""
        text_parts = [
...
class ZepToolsService:
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        # LLM client is used by InsightForge to generate sub-problems
    ...
    def get_simulation_context(
        ...
        return {
            ...
            "entities": entities[:limit], # limit the number
    ...
    def insight_forge(
        ...
        # Get the details of all related entities (no limit on the number, complete output)
        entity_insights = []
        ...
        for uuid in list(entity_uuids): # Process all entities without truncation
            ...
            try:
                ...
                logger.debug(f"Failed to obtain node {uuid}: {e}")
                continue
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are a professional problem analysis expert. Your task is to decompose a complex problem into multiple sub-problems that can be independently observed in the simulated world.
...
Returns a list of subquestions in JSON format. """
        ...
        try:
            ...
            # Downgrade: Return a variant based on the original problem
            return [
                query,
    ...
    def panorama_search(
        ...
        for edge in all_edges:
            ...
            # Determine whether it is expired/invalid
    ...
    def interview_agents(
        ...
        # Step 4: Call the real interview API (without specifying platform, the default is to interview on both platforms simultaneously)
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Get the interview results of the Agent on the two platforms
                ...
                twitter_text = twitter_response if twitter_response else "(The platform did not get a reply)"
                reddit_text = reddit_response if reddit_response else "(The platform did not get a reply)"
                ...
                # Clean up the response text: remove tags, numbers, Markdown and other interference
                ...
                # Strategy 2 (Supplementary): Correctly matched Chinese quotation marks "" within long text
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may be closed, please ensure that the OASIS environment is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean the JSON tool call package in the Agent reply and extract the actual content"""
...
Please select up to {max_agents} agents that are most suitable for interviews and explain the reasons for your selection. """
        ...
        try:
            ...
            # Get the complete information of the selected Agent
            selected_agents = []
            valid_indices = []
...
Please generate 3-5 interview questions. """
        ...
        try:
            ...
            return response.get("questions", [f"What do you think about {interview_requirement}?"])
