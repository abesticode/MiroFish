...
class AgentActivity:
    ...
    def _describe_like_post(self) -> str:
        """Like the post - including the original text and author information"""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike the post - including the original text and author information"""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_repost(self) -> str:
        """Repost - include original post content and author information"""
    ...
    def _describe_quote_post(self) -> str:
        """Quote posts - include original post content, author information and quoted comments"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_follow(self) -> str:
        """Following User - Contains the name of the followed user"""
    ...
    def _describe_create_comment(self) -> str:
        """Post a comment - include comment content and information about the post being commented on"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_dislike_comment(self) -> str:
        """Dislike comments - including comment content and author information"""
        comment_content = self.action_args.get("comment_content", "")
        comment_author = self.action_args.get("comment_author_name", "")
    ...
    def _describe_generic(self) -> str:
        # For unknown action types, generate a general description
...
class ZepGraphMemoryUpdater:
    ...
    # Batch sending size (how many items are accumulated on each platform before sending)
    BATCH_SIZE = 5
    ...
    #Sending interval (seconds) to avoid requesting too quickly
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        self._total_items_sent = 0 # Number of activity items successfully sent to Zep
        self._failed_count = 0 # Number of failed batches to send
    ...
    def stop(self):
        ...
        logger.info(f"ZepGraphMemoryUpdater has stopped: graph_id={self.graph_id}, "
    ...
    def _worker_loop(self, locale: str = 'zh'):
        """Background work loop - batch sending activities to Zep by platform"""
        set_locale(locale)
        while self._running or not self._activity_queue.empty():
            try:
                # Attempt to get activity from queue (timeout 1 second)
                try:
                    activity = self._activity_queue.get(timeout=1)
                    
                    # Add the activity to the buffer of the corresponding platform
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logger.info(f"Successfully sent {len(activities)} {display_name} activities to graph {self.graph_id}" in batches)
                ...
                if attempt < self.MAX_RETRIES - 1:
                    ...
                    logger.error(f"Batch sending to Zep failed, retried {self.MAX_RETRIES} times: {e}")
                    self._failed_count += 1
    
    def _flush_remaining(self):
        """Remaining activity in send queue and buffer"""
        ...
        # Then send the remaining activities in the buffer of each platform (even if there are less than BATCH_SIZE items)
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(f"Send the remaining {len(buffer)} activities of {display_name} platform")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "items_sent": self._total_items_sent, # Number of successfully sent activity items
...
class ZepGraphMemoryManager:
    ...
    def stop_updater(cls, simulation_id: str):
        """Stop and remove the simulated updater"""
        with cls._lock:
            if simulation_id in cls._updaters:
                cls._updaters[simulation_id].stop()
    ...
    # Flag to prevent stop_all from being called repeatedly
    ...
    def stop_all(cls):
        ...
        with cls._lock:
            if cls._updaters:
                ...
                cls._updaters.clear()
            logger.info("All map memory updaters have been stopped")
